...
import { generateWithLLM, type LLMSongData } from '../services/llm/index.js';
import { radioManager } from '../services/radioState.js';
import { processAudioWithSettings } from '../services/audioProcessor.js';
...
interface GenerateBody {
  ...
  // Username (replaces JWT auth)
  username?: string;

  // Instruct Mode (Claude enhancement)
...
router.post('/', authMiddleware, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const {
      customMode,
      songDescription: rawSongDescription,
      lyrics,
      style,
      title,
      instrumental,
    ...
    let songDescription = rawSongDescription;

    // Get instruct params from request body
    const { instructMode, instructParams, llmData: passedLLMData } = req.body as GenerateBody;

    const isVibesMode = instructMode && passedLLMData;

    if (!customMode && !songDescription && !isVibesMode) {
      res.status(400).json({ error: 'Song description required for simple mode' });
      return;
    }

    if (customMode && !style && !lyrics && !isVibesMode) {
      res.status(400).json({ error: 'Style or lyrics required for custom mode' });
      return;
    }

    // Check if LLM is enabled in radio settings
    ...
    console.log('[Generate] Request received:', {
      ...
      instructEffects: instructParams?.effects,
      customInstruction: instructParams?.customInstruction?.slice(0, 50),
      hasLLMData: !!passedLLMData,
      llmDataTitle: passedLLMData?.song_title,
      useLLM,
      llmProvider: radioSettings.llmProvider,
      user: req.user?.username,
    });

    // Variables that may be enhanced by LLM
    let enhancedLyrics = lyrics;
    let enhancedStyle = style;
    let enhancedTitle = title;
    let enhancedDuration = duration;
    let enhancedBpm = bpm;
    let enhancedKeyScale = keyScale;
    let enhancedTimeSignature = timeSignature;
    let llmResponse: LLMSongData | null = null;

    // If LLM is enabled, process through LLM first
    if (useLLM) {
      console.log('[Generate] LLM is enabled, processing request...');
      try {
        const parts: string[] = [];

        if (instructMode && instructParams) {
          if (instructParams.effects && instructParams.effects.length > 0) {
            parts.push(`Make it: ${instructParams.effects.join(', ')}`);
          }
          if (instructParams.customInstruction) {
            parts.push(instructParams.customInstruction);
          }
        } else {
          if (songDescription) {
            parts.push(songDescription);
          } else if (style) {
            parts.push(style);
          }
          if (title) {
            parts.push(`Song title: ${title}`);
          }
          if (lyrics) {
            parts.push(`Lyrics:\n${lyrics}`);
          }
        ...
        if (vocalLanguage && vocalLanguage !== 'en' && vocalLanguage !== 'unknown') {
          parts.push(`Vocal language: ${vocalLanguage}`);
        }

        const userPrompt = parts.join('\n\n');
        console.log('[Generate] LLM user prompt:', userPrompt.slice(0, 200));

        const previousData = instructMode ? (passedLLMData || {
          song_title: title || 'Untitled',
          prompt: style || songDescription || '',
          lyrics: lyrics || '',
          audio_duration: duration && duration > 0 ? duration : undefined,
          bpm: bpm && bpm > 0 ? bpm : undefined,
          key_scale: keyScale || undefined,
          time_signature: timeSignature || undefined,
        }) : undefined;

        const providerSettings = radioManager.getLLMProviderSettings();

        const generated = await generateWithLLM({
          userPrompt,
          isInstructMode: instructMode,
          previousData,
          ...providerSettings,
        });

        console.log('[Generate] LLM response received, applying enhancements...');
        if (generated.song_title) enhancedTitle = generated.song_title;
        if (generated.prompt) {
          enhancedStyle = generated.prompt;
          // Also update songDescription so the enhanced prompt flows through
          // in simple mode (buildMusicRequest prefers songDescription when !customMode)
          songDescription = generated.prompt;
        }
        if (generated.lyrics !== undefined) enhancedLyrics = generated.lyrics;
        if (generated.audio_duration) enhancedDuration = generated.audio_duration;
        if (generated.bpm) enhancedBpm = generated.bpm;
        if (generated.key_scale) enhancedKeyScale = generated.key_scale;
        if (generated.time_signature) enhancedTimeSignature = generated.time_signature;

        llmResponse = generated;

        radioManager.updateGenParams({
          customMode,
          songDescription,
          ...
          instrumental,
          vocalLanguage,
          instructMode: instructMode || false,
          instructParams: instructParams || undefined,
          llmData: generated,
        });
      } catch (llmError) {
        console.error('[Generate] LLM processing failed, using fallback params:', llmError);
        if (instructMode && passedLLMData) {
          if (passedLLMData.song_title) enhancedTitle = passedLLMData.song_title;
          if (passedLLMData.prompt) {
            enhancedStyle = passedLLMData.prompt;
            songDescription = passedLLMData.prompt;
          }
          if (passedLLMData.lyrics !== undefined) enhancedLyrics = passedLLMData.lyrics;
          if (passedLLMData.audio_duration) enhancedDuration = passedLLMData.audio_duration;
          if (passedLLMData.bpm) enhancedBpm = passedLLMData.bpm;
          if (passedLLMData.key_scale) enhancedKeyScale = passedLLMData.key_scale;
          if (passedLLMData.time_signature) enhancedTimeSignature = passedLLMData.time_signature;
        }
      }
    }
    ...
    const params = {
      ...
      bpm: enhancedBpm,
      keyScale: enhancedKeyScale,
      timeSignature: enhancedTimeSignature,
      inferenceSteps,
      guidanceScale,
      batchSize,
      randomSeed,
      seed,
      thinking: useLLM ? false : thinking,
      audioFormat,
      inferMethod,
      shift,
      lmTemperature,
    ...
    // Start generation
    console.log('[Generate] Final params for ACE-Step:', {
      title: params.title,
    ...
    await pool.query(
      `UPDATE generation_jobs SET provider_task_id = ?, acestep_task_id = ?, status = 'running', updated_at = datetime('now') WHERE id = ?`,
      [hfJobId, hfJobId, localJobId]
    );
...
router.get('/status/:jobId', authMiddleware, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const jobResult = await pool.query(
      `SELECT id, user_id, COALESCE(provider_task_id, acestep_task_id) as provider_task_id, acestep_task_id, status, params, result, error, created_at
    ...
    // If job is still running, check provider status
    const taskId = job.provider_task_id || job.acestep_task_id;
    if (['pending', 'queued', 'running'].includes(job.status) && taskId) {
      try {
        const aceStatus = await getJobStatus(taskId);
            ...
            for (let i = 0; i < audioUrls.length; i++) {
              ...
              try {
                ...
                buffer = await processAudioWithSettings(buffer, {
                  audioPostProcess: radioSettings.audioPostProcess,
                  audioHighPass: radioSettings.audioHighPass,
                  audioLowPass1: radioSettings.audioLowPass1,
                  audioLowPass2: radioSettings.audioLowPass2,
...
router.get('/history', authMiddleware, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const result = await pool.query(
      `SELECT id, COALESCE(provider_task_id, acestep_task_id) as provider_task_id, acestep_task_id, status, params, result, error, created_at
       FROM generation_jobs
       WHERE user_id = ?
...
router.get('/endpoints', async (_req, res: Response) => {
  try {
...
router.get('/debug/:taskId', async (req, res: Response) => {
  try {
...
router.post('/format', authMiddleware, async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    const ACESTEP_DIR = process.env.ACESTEP_PATH || path.resolve(path.dirname(fileURLToPath(import.meta.url)), '../../../models/ACE-Step-1.5');
...
// Audio analysis endpoint - uses vLLM multimodal model to analyze audio
router.post('/analyze-audio', authMiddleware, audioUpload.single('audio'), async (req: AuthenticatedRequest, res: Response) => {
  try {
    if (!req.file) {
      res.status(400).json({ error: 'Audio file is required' });
      return;
    ...
    if (!radioSettings.useLLM) {
      res.status(400).json({ error: 'AI is not enabled' });
      return;
    }

    if (radioSettings.llmProvider !== 'vllm' || !radioSettings.llmVllmSettings?.supportsAudio) {
      res.status(400).json({ error: 'Audio analysis requires a vLLM provider with audio support enabled' });
      return;
    }

    console.log('[Generate] Audio analysis request:', {
    ...
    if (!result || (!result.song_title && !result.prompt && !result.lyrics)) {
      res.status(500).json({ error: 'Audio analysis returned no useful data' });
      return;
    }

    console.log('[Generate] Audio analysis result:', {
    ...
    res.json({ llmData: result });
  } catch (error) {
    console.error('Audio analysis error:', error);
    res.status(500).json({ error: (error as Error).message || 'Audio analysis failed' });
  }
});
